...
- **✅✅ PRIX TOTAL DYNAMIQUE (nouveau modèle Faouez)** (migration 0077 + app) : le conducteur met **UN prix total** pour le voyage (plus « À convenir »). 2 modes = **Offert** (price_total 0) / **Participation** (price_total >0). Colonne `trips.price_total`. Fonctions `trip_seat_price` (prix pour un passager qui rejoint = total ÷ (conducteur + confirmés + lui)) et `trip_current_seat_price`. `preview_trip_cost` renvoie `total_suggested` (coût réel du voyage) + `total_max` (× marge). Create screen : le conducteur saisit le total (pré-rempli suggéré, plafonné), avec aperçu « avec N passagers + toi = X DT chacun ». Affichage (TripCard + détail) via `seatPriceNow()` = **prix dynamique par personne** (baisse quand la voiture se remplit). `tsc`=0. Ex Tunis–Sousse : total suggéré 37 · max 70.
  - **✅ NOTIF BAISSE DE PRIX + places restantes** (migration 0078) : `seats_available` maintenu = places restantes (trigger `trg_bookings_seats` recompute sur accepted ; **bug latent corrigé** : jamais décrémenté avant → masque aussi les trajets pleins). Trigger `notify_price_drop` : quand un passager est accepté sur un trajet Participation, push aux AUTRES passagers acceptés (« ton prix baisse, un passager a rejoint, ta part = X »). **Testé en rollback** : 100 DT → 50 (÷2) → 33,33 (÷3) → 25 (÷4), places 3→2→1→0. ⚠️ la notif ne part que si le passager a un push token (setup Expo déjà là, 0006).
  - Reste pour compléter le modèle : (a) **facture/reçu** en fin de voyage (total ÷ N) ; (b) aligner le SITE.
- **✅ CÔTÉ PASSAGER = prix choisi UNIQUEMENT (décision Faouez)** : le passager voit **seulement le prix demandé par le conducteur** (badge `formatPrice`), **PAS** le calcul détaillé (carburant/usure/péage). Plus protecteur légalement (participation informelle, pas une facture de transport calculée). Le détail transparent qu'on avait ajouté a été **retiré** de `trip/[id].tsx` (hook `useTripCost` conservé dans pricing.ts, inutilisé pour l'instant). Le calcul reste **côté conducteur** (suggestion + plafond à la création). `tsc`=0.
