...
| 7-bis | Vérification modérée pour tous (CIN + permis + carte grise) | ✅ | KYC bucket privé + écran upload + RLS. **Modération DANS l'app (2026-07-02, migration 0037)** : `profiles.is_admin`, écran `/profile/kyc-review` (dossiers par user, aperçu signé, Approuver/Refuser), RPC `admin_review_kyc` recalcule le badge Vérifié selon le rôle (conducteur = cin+permis+carte_grise+photo_vehicule, passager = cin). **Verrou publication F10 `kyc_publish_gate`** (OFF défaut) : conducteur non vérifié ne peut pas publier (trigger serveur + UI). ⚠️ Donner l'admin à Faouez : `update profiles set is_admin=true where phone='+216...'`. |
...
| 10 | Petites annonces + **suggestions intelligentes + waypoint matching** | 🟡 | Annonces + recherche PostGIS ✅. **Carte trajet ✅ (2026-06-12)** — ⚠️ crashait en build standalone sans clé Google Maps ; **corrigé 2026-07-02** : clé API Maps ajoutée (`app.json` android.config.googleMaps, projet GCloud `machii-501202`) + chargement paresseux react-native-maps avec repli schéma. Vraie carte OK. Waypoint matching (via, détour, suggestions) ⏸️ — la table `trip_waypoints` existe, jamais branchée. |
